...
class MTCNNDetector(object):
    ...
    def create_mtcnn_net(self, p_model_path=None, r_model_path=None, o_model_path=None, use_cuda=True):
        ...
        if p_model_path is not None:
            pnet = PNet()
            pnet.set_state_dict(paddle.load(p_model_path))
            pnet.eval()

        if r_model_path is not None:
            rnet = RNet()
            rnet.set_state_dict(paddle.load(r_model_path))
            rnet.eval()

        if o_model_path is not None:
            onet = ONet(train_landmarks=True)
            onet.set_state_dict(paddle.load(o_model_path))
            onet.eval()
    ...
    def detect_rnet(self, im, bboxes):
        ...
        # there might be OOM
        feed_imgs = paddle.stack(cropped_ims_tensors)
